...
def intervalo_horas_2(hora_inicio, hora_final, periocidades, data_inicio):
    data = []
    data2 = []
    ...
    for h in range(int(hinicio[0]), int(hfinal[0])+1):
        ...
        for p in perio:
            perioc.append(p.id)
        reserva = Reserva.objects.filter(periocidadeid__in=perioc,  preco__isnull = False)
        for r in reserva:
            sum += r.preco
            sum2 += r.divida
        data.append(sum)
...
def intervalo_dias_2(data_inicio, data_final, periocidades):
    data = []
    data2 = []
    label = []
    dateStart = datetime.strptime(data_inicio,"%Y-%m-%d").date()
...
def intervalo_meses_2(data_inicio, data_final, periocidades):
    data=[]
    data2 = []
    label=[]
    dateStart = datetime.strptime(data_inicio,"%Y-%m-%d").date()
...
def intervalo_anos_2(data_inicio, data_final, periocidades):
    data=[]
    data2=[]
    label=[]
    dateStart = datetime.strptime(data_inicio,"%Y-%m-%d").date()
...
def intervaloTempo_2(data_inicio, hora_inicio, data_final, hora_final, periocidades):
    dateStart = datetime.strptime(data_inicio,"%Y-%m-%d").date()
    dateEnd = datetime.strptime(data_final,"%Y-%m-%d").date()
    d = dateEnd - dateStart
    if (d.days < 1): #ver em intervalo de horas
        return intervalo_horas_2(hora_inicio, hora_final, periocidades, data_inicio)
    elif (d.days <= 31): #ver em intervalo de dias
        return intervalo_dias_2(data_inicio, data_final, periocidades)
    elif (d.days<=365): #ver em intervalo de meses
        return intervalo_meses_2(data_inicio, data_final, periocidades)
    else: #ver em anos
        return intervalo_anos_2(data_inicio, data_final, periocidades)
